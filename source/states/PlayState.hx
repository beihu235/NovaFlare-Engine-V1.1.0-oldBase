...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		dad = new Character(0, 0, SONG.player2, ClientPrefs.data.playOpponent);
		startCharacterPos(dad, true);
		dadGroup.add(dad);
		startCharacterScripts(dad.curCharacter);

		boyfriend = new Character(0, 0, SONG.player1, !ClientPrefs.data.playOpponent);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				swagNote.mustPress = ClientPrefs.data.playOpponent ？!gottaHitNote : gottaHitNote;
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus+1)
					{
						...
						sustainNote.mustPress = ClientPrefs.data.playOpponent ？!gottaHitNote : gottaHitNote;
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		{
			// FlxG.log.add(i);
			var targetAlpha:Float = 1;//修改
			if ((player == 0 && !ClientPrefs.data.playOpponent) || (player == 1 && ClientPrefs.data.playOpponent))
			{
						...
						notes.forEachAlive(function(daNote:Note)
						{
							...
							if(daNote.mustPress)
							{
								if(cpuControlled && !daNote.blockHit && daNote.canBeHit && (daNote.isSustainNote || daNote.strumTime <= Conductor.songPosition))
									ClientPrefs.data.playOpponent ? opponentNoteHitForOpponent(daNote) : goodNoteHit(daNote);
							}
							else if (daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
								    ClientPrefs.data.playOpponent ？goodNoteHitForOpponent(daNote) : goodNoteHitopponentNoteHit(daNote);
	...
	function opponentNoteHitForOpponent(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(!cpuControlled)
			{
				var spr = playerStrums.members[note.noteData];//修改
	...
	public function spawnNoteSplashOnNote(note:Note) {
		if(note != null) {
			var strum:StrumNote = playerStrums.members[note.noteData];//修改
			if(strum != null)
	...
	function strumPlayAnim(isDad:Bool, id:Int, time:Float) {//修改
