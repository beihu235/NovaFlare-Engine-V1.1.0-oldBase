				...
				if (selectedOption.acceptType || !selectedOption.acceptType)
				{
					...
					if (down || down_hold)
					{
						...
						if (selectedOptionIndex != 0
							...
							&& options[selectedCatIndex].options.length > 8)
						{
					...
					else if (up || up_hold)
					{
						...
						if (selectedOptionIndex < 0)
						{
							...
							if (options[selectedCatIndex].options.length > 8)
								for (i in selectedCat.optionObjects.members)
								{
									i.y -= (46 * (Math.floor(options[selectedCatIndex].options.length - 1 + 0.5) / 2));
								}
						}
